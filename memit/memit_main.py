...
def execute_memit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        try:
            ...
            identity_matrix = torch.eye(layer_ks.size(0)).to('cuda:1') * (1e-4)
        except:
            layer_ks, targets = (
                layer_ks.double(),
                targets.double(),
            )
            identity_matrix = torch.eye(layer_ks.size(0)).to('cuda') * (1e-4)
        adj_k = torch.linalg.solve(
            hparams.mom2_update_weight * cov.double() + layer_ks @ layer_ks.T + identity_matrix,
            layer_ks,
        )
