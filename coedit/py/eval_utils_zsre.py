...
def compute_rewrite_quality_zsre(
    ...
    # print(f"target_tok: {target_tok} \n  inp_targets: {inp_targets} \n inp_prompts: {inp_prompts}")
...
def test_batch_prediction_acc(model, tok, prompts: typing.List[str], target):
    ...
    with torch.no_grad():
        ...
        ans_tok = tok.batch_decode(ans, skip_special_tokens=True)
        # print(f"Ans Tok: {ans_tok}")
        # print(f"Ans: {ans}")
        correct_id = tok(target, padding=True, add_special_tokens=False, return_tensors="pt").to("cuda")[
            "input_ids"
        ]
